...
spark = SparkSession.builder.config("spark.mongodb.input.uri", mongo_conn)\
    ...
    .config("spark.mongodb.output.socketTimeoutMS", "30000") \
    .config("spark.mongodb.connectionTimeoutMS", "45000")\
    .getOrCreate()
...
file_path_inicial = '/data/version_noviembre_reglas_inicial_v2.json'
#file_path_inicial = '/data/version_agosto_reglas_inicial_v2.json'
#reglas_vencida_vigente_inicial_V2025.json'
#file_path_goteo = '/data/reglas_vencida_vigente_inicial_V2025.json'
#file_path_inicial = '/data/reglas_vencida_vigente_goteo_V2025.json'
#file_path_goteo = '/data/reglas_vencida_vigente_inicial_V2025_diario.json'
...
'''if "Sub_estrategia" in df.columns:
    ...
    df_goteo = df.filter(
        ...
        (~((col("Sub_estrategia") == "C033") | (col("Sub_estrategia") == "F037")) | (col("Sub_estrategia").isNull()))
    )
else:'''
df_inicial = df

# Borrar las columnas específicas
df_inicial = df_inicial.drop("regla", "Sub_estrategia", "Canal", "Transicionan", "Comentarios","agencia")
df_inicial = df_inicial.withColumn("agencia", lit(""))
#df_goteo = df_goteo.drop("regla", "Sub_estrategia", "Canal", "Transicionan", "Comentarios")
...
df_inicial.write.format("mongo").option("collection",coll_output).mode("overwrite").save()
